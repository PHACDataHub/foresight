...
def stat_runner(func):
    ...
    def wrap(*args, **kwargs): 
        ...
        if total > 0:
            print(f"{func.__name__} --- {seconds:02f} secs --- {total} items --- {seconds/total:0.2f} secs per item.", flush=True)
        return result 
    return wrap 
...
class GeoLocator():
    ...
    def __init__(self, config_section):
        ...
        self.geocoder = googlemaps.Client(key=config_section['google_api_key'])
        self.call_counter = 0
...
class MachineTranslator():
    ...
    # def __init__(self, config_section, device):
    #     self.text_splitter = RecursiveCharacterTextSplitter(separators=["\n\n", "\n"], chunk_size=200)
    #     self.language_models = dict()
    #     for src_lang, model_name in eval(config_section['language_models']):
    #         self.language_models[src_lang] = model_name
    #     self.device = device
    ...
    def __init__(self, config_section):
        ...
        for lang_id, lang_name in eval(config_section['flores_languages']):
            self.language_dict[lang_id] = lang_name
        self.llm_name = config_section['llm']
        # model_llm = Ollama(model=config_section['llm'], temperature=0.0)
        
        # output_parser = NumberedListOutputParser()
        # format_instructions = output_parser.get_format_instructions()
        # PROMPT_TEMPLATE = """Translate {src_lang_name} to English:
        # ```{text}```
        
        # {format_instructions}:"""
        # prompt = PromptTemplate(
        #     input_variables=["text", "src_lang_name"], 
        #     partial_variables={"format_instructions": format_instructions}, 
        #     template=PROMPT_TEMPLATE)

        # self.llm_chain = LLMChain(llm=model_llm, prompt=prompt)
        self.stat_dict = dict()
    
    def generate(self, prompt):
        response = requests.post(
            'http://localhost:11434/api/generate', 
            data=json.dumps({"model": self.llm_name, "prompt": prompt, "stream": False,}), 
            headers={"Content-Type": "application/json"})
        
        if response.status_code == 200:
            result = json.loads(response.text)['response'].split('\n\n')
            return '\n'.join(result[1:] if 'he translation' in result[0] else result)
        else:
            print(f"Request failed with status code {response.status_code}")
            return None
    
    def translate(self, src_lang, text):
        start_time = datetime.now()

        result = text
        if src_lang and src_lang != 'eng_Latn':
            src_lang_name = self.language_dict[src_lang]
            result = self.generate(f"Translate {src_lang_name} to English: {text}")

        if src_lang not in self.stat_dict:
            self.stat_dict[src_lang] = [0, 0]

        end_time = datetime.now()
        seconds = (end_time - start_time).total_seconds()
        self.stat_dict[src_lang][0] += 1
        self.stat_dict[src_lang][1] += seconds

        return result is not None, result
    
    def print_stats(self):
        for src_lang in sorted(self.stat_dict.keys()):
            nr = self.stat_dict[src_lang][0]
            secs = self.stat_dict[src_lang][1]
            print(f"{src_lang} --- {nr:05d}] in {secs:02f} secs --- {secs/nr:0.2f} secs per item.", flush=True)
        ...
        for d in documents:
            if 'title' not in d or not d['title']:
                ...
                if 'content' in d and d['content']:
                    d['summary'] = d['content']
                    d['content'] = None
        texts = ['\n\n'.join([d[k] for k in ['title', 'summary', 'content'] if k in d and d[k]]) for d in documents]
        embeddings = embedding_model.encode(texts, show_progress_bar=True)
        total = len(documents)
        return total, [embeddings, texts, documents]


def load_batches(in_path, file_patterns, embedding_model):
    batches = []
    for file_name in sorted(os.listdir(in_path)):
        in_file_name = os.path.join(in_path, file_name)
        if not os.path.isfile(in_file_name):
            continue
        if all(p in in_file_name for p in file_patterns):
            embeddings_texts_documents = load_batch(in_file_name, embedding_model)
            batches.append(embeddings_texts_documents)
    return batches
   

def create_clustering_tools(embedding_model):
...
@stat_runner
def cluster_batch(batch, embedding_model):
    topic_model = create_clustering_tools(embedding_model)
    embeddings, texts, _ = batch
    topic_model.fit_transform(texts, embeddings)
    return len(texts), topic_model


def cluster_batches(batches, embedding_model, print_topics=False):
    topic_model = None
    for batch in batches:
        batch_model = cluster_batch(batch, embedding_model)
...
def predict_batch(topic_model, batch, topic_dict):
    ...
    for topic, prob, document, text in zip(topics, probs, documents, texts):
        topic_id = int(topic)
        if topic_id not in topic_dict:
            topic_id = -1
        doc_id = document['id']
        doc_lang = document['language'][0] if 'language' in document else 'eng_Latn'
        topic_dict[topic_id]['documents'][doc_id] = [float(1.0) if prob > 1.0 else float(prob), text, doc_lang]
    return len(topic_dict), topic_dict
    

@stat_runner
def predict_batches(topic_model, batches, topic_dict, machine_translator):
    nr_documents = 0
    title_dict = dict()
    for batch in batches:
        _, _, documents = batch
    ...
    for topic_id in sorted(topic_dict.keys()):
        ...
        langs = []
        for doc_id, prob_text_lang in sorted_doc_prob:
            prob, text, lang = prob_text_lang
            if title_dict[doc_id] in distinct_titles:
                continue
            if prob == 1.0 or (prob < 1.0 and len(representatives) < NR_REPR_DOCS):
                langs.append(lang)
                result, translated_text = machine_translator.translate(lang, text)
                if result:
                    print(f"[{topic_id}] {lang} --- {translated_text.replace('\n', ' ')}")
                    text = translated_text
                representatives.append([doc_id, prob, text, lang])
                distinct_titles.add(title_dict[doc_id])
            else:
                break
        topic_dict[topic_id]['representatives'] = representatives
        print(f"[{topic_id}] --- {langs}")
        # machine_translator.print_stats()
...
def filter_texts(info, llm_model, max_nr_tokens, max_repr_docs):
    ...
    for _, _, text, _ in info['representatives'][:min_nr_docs]:
        tr_text = text
...
if __name__ == '__main__':
    ...
    in_path, country_code_file = sys.argv[2], sys.argv[3]

    country_dict = load_country_codes(country_code_file)
    geolocator = GeoLocator(config['output'])
    ...
    machine_translator = MachineTranslator(config['output'])

    # batches = load_batches(in_path, date_list, start_date, end_date, embedding_model)
    batches = load_batches(in_path, eval(config['output']['file_patterns']), embedding_model)
    
    # topic_model = cluster_batches(batches, embedding_model, seed_phrases, print_topics=True)
    topic_model = cluster_batches(batches, embedding_model, print_topics=True)
    ...
    topic_dict = predict_batches(topic_model, batches, topic_dict, machine_translator)
    for topic_id in sorted(topic_dict.keys()):
        print(f"[{topic_id}] --- [{len(topic_dict[topic_id]['documents'])}] --- {len(topic_dict[topic_id]['representatives'])} --- {topic_dict[topic_id]['name']} --- {topic_dict[topic_id]['keywords']}")
    ...
    sim_file_name = f"{in_path}/processed-sim.jsonl"
    save_jsonl(similarity_dict, sim_file_name, single=True)

    tps_file_name = f"{in_path}/processed-tps.jsonl"
    ...
    lbl_file_name = f"{in_path}/processed-lbl.jsonl"
    ...
    cls_file_name = f"{in_path}/processed-cls.jsonl"
