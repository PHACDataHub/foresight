...
def performing_tasks(temp_path, out_path, period, documents, similarity_threshold, content_similarity_ratio):
    # Compute similarity between articles in each of the clusters
    loc_file_name = f"{temp_path}/processed-{period}-cls-ctm.jsonl"
    ...
    smf_file_name = f"{out_path}/processed-{period}-smf-ctm.jsonl"
    save_jsonl(topic_dict, smf_file_name, single=True)


if __name__ == '__main__':
    in_path, temp_path, out_path, start_date, end_date = sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[4], sys.argv[5]
    if len(sys.argv) > 7:
        similarity_threshold, content_similarity_ratio = float(sys.argv[6]), float(sys.argv[7])
    else:
    ...
    for pub_date in date_list:
        ...
        doc_file_name = f"{in_path}/processed-{pub_date}-news-articles.jsonl"
        ...
        # performing_tasks(temp_path, out_path, pub_date, daily_documents, similarity_threshold, content_similarity_ratio)
    ...
    for period_length, period_days in [ (3, range(30, 37)), (7, range(30, 31)), (30, range(30, 31)) ]:
        for i in period_days:
            ...
            performing_tasks(temp_path, out_path, period, period_documents, similarity_threshold, content_similarity_ratio)
